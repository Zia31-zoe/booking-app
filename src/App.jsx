                  ...
                  {editingId === p.id ? (
                    <span className="payment-edit-amount-row">
                      <input
                        ...
                        autoFocus
                      />
                      <button type="button" className="icon-btn" onClick={() => saveEdit(p)} title="儲存">
                        <CheckCircle2 size={15} />
                      </button>
                      <button type="button" className="icon-btn" onClick={() => setEditingId(null)} title="取消">
                        <X size={15} />
                      </button>
                    </span>
                  ) : (
                    <span className="payment-amount-trigger" onClick={() => startEdit(p)} title="點擊編輯金額">
                      {fmtMoney(p.amount)}
...
function CalendarView({ members, sessions, blockedTimes, isStaff, currentCoachId, coaches, onOpenSession, onAddBlockedTime, onDeleteBlockedTime, searchQuery, onSearchChange }) {
  ...
  // 包含「排定中」與「已完成」的課堂,讓行事曆能完整呈現一週實際上課狀況
...
function CashflowManager({ members, sessions, payments, manualCharges, venues, coaches, isStaff, currentCoachId, onAddVenue, onUpdateVenue, onDeleteVenue, onPurchaseVenue }) {
  ...
  const availableYears = useMemo(() => {
    const years = new Set();
    payments.forEach((p) => {
      if (p.paid_date) years.add(p.paid_date.slice(0, 4));
      else if (p.due_date) years.add(p.due_date.slice(0, 4));
    });
  ...
  return (
    <div className="page">
      ...
      {(filterYear || filterMonth) && (
        <div className="hint-box">
          「已收金額」「已收」相關統計目前僅顯示 {filterYear ? `${filterYear}年` : ""}{filterMonth ? `${Number(filterMonth)}月` : ""} 實際收款的紀錄,其餘課費總額、未收金額等統計不受篩選影響。
        </div>
      )}
