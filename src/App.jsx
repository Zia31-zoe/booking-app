...
function Overview({ members, sessions, payments, blockedTimes, onGoSchedule, onOpenSession, onOpenMember }) {
  ...
  const stats = useMemo(() => {
    ...
    // 本週已安排課堂數(涵蓋排定中與已完成,不含已取消)
    const weeklyScheduled = sessions.filter(
      (s) => s.status !== "cancelled" && s.session_date >= weekStart && s.session_date <= weekEnd
    ).length;
    ...
    return { pending, completed, totalRemaining, activeMemberCount, monthlyTrials, monthlyConverted, weeklyScheduled, weeklyAbsent };
  }, [members, sessions, today, weekStart, weekEnd]);

  // 今天的課堂(排定中,含請假,依時間排序;請假的課堂會以灰色顯示)
  ...
  // 今天的不可排課時段(單次日期符合今天,或每週固定符合今天的星期)
  const todayBlocks = useMemo(() => {
    const wd = weekdayOf(today);
    return (blockedTimes || []).filter(
      (b) => (b.block_date && b.block_date === today) || (b.weekday !== null && b.weekday !== undefined && Number(b.weekday) === wd)
    );
  }, [blockedTimes, today]);
    ...
    <div className="page">
      ...
      <div className="stat-grid">
        ...
        <div className="stat-card">
          ...
          <span className="stat-value">{stats.weeklyScheduled}</span>
        </div>
        ...
        <div className="overview-timeline-wrap">
          <div className="overview-timeline" style={{ height: `${15 * CAL_H}px` }}>
            ...
            {todayBlocks.map((b, i) => {
              ...
              const height = Math.max(timeToTop(b.end_time) - top, 20);
              return (
                <div
                  key={`block-${b.id || i}`}
            ...
            {todaySessions.map((s) => {
              ...
              return (
                <button
                  key={s.id}
                  className={`ovt-block ${isTrial ? "trial" : "member"} ${isAbsent ? "absent" : ""}`}
                  style={{
                    top: `${top}px`,
                    height: `${height}px`,
                    ...(isAbsent ? { background: "#9ca3af", borderColor: "#6b7280", color: "#fff" } : {}),
                  }}
                ...
                >
                  <span className="ovt-block-name">
                    ...
                    {isAbsent && <span className="pill" style={{fontSize:"9px",padding:"0 4px",background:"#6b7280",color:"#fff"}}>請假</span>}
            ...
            {todaySessions.length === 0 && todayBlocks.length === 0 && (
...
function ScheduleManager({ members, sessions, onAdd, onOpenSession, presetMemberId, onPresetUsed, venues, blockedTimes, searchQuery, onSearchChange, globalStats }) {
  ...
  const upcoming = useMemo(() => {
    ...
    return sessions
      // 已標記請假或已完成的課堂,不再顯示於待安排/待完成清單中
      .filter((s) => s.status === "scheduled" && !s.is_absent)
      .filter((s) => {
        if (!q) return true;
...
function MembersManager({
  ...
  const allRegularMembers = members.filter((m) => m.member_type === "member" && matchesSearch(m));
  // 有剩餘堂數的學員排在上方,無堂數的排在最下方
  const regularMembers = allRegularMembers.filter((m) => Math.max(m.total_sessions - m.used_sessions, 0) > 0);
  const depletedMembers = allRegularMembers.filter((m) => Math.max(m.total_sessions - m.used_sessions, 0) <= 0);
    ...
    <div className="page">
      ...
      <div className="section">
        ...
        <div className="member-grid">
          {regularMembers.length === 0 && depletedMembers.length === 0 && <div className="empty-block">尚無學員,點上方「新增學員」開始建立。</div>}
      ...
      {depletedMembers.length > 0 && (
        <div className="section">
          ...
          <div className="member-grid">
            {depletedMembers.map((m) => {
              const today = todayStr();
              const inbody = inbodyReminderStatus(m, today);
              const period = menstrualReminderStatus(m, today);
              return (
                <div className="member-card" key={m.id} style={{ opacity: 0.7 }}>
                  <div className="member-card-head">
                    <button className="member-name-btn" onClick={() => setDetailFor(m)}>
                      <span className="member-name">{m.name}</span>
                    </button>
...
function MemberDetailModal({
  ...
  // 請假紀錄:該學員被標記為請假的課堂,依日期新到舊排序
  const absentSessions = sessions
          ...
          {tab === "history" && (
            <div className="section">
              <div className="stat-grid">
                ...
                <div className="stat-card">
                  <span className="stat-label">請假堂數</span>
                  <span className="stat-value warn">{absentSessions.length}</span>
                </div>
              </div>

              <h4 className="section-title-sm">已完成課堂紀錄</h4>
              ...
              <h4 className="section-title-sm">請假紀錄</h4>
              {absentSessions.length === 0 ? (
                <div className="empty-block small">尚無請假紀錄。</div>
              ) : (
                <div className="list">
                  {absentSessions.map((s) => (
                    ...
                    >
                      <div className="history-row-head">
                        <span className="list-row-title">
                          {fmtDate(s.session_date)} · {(s.start_time || "").slice(0, 5)}
                          {s.location ? ` · ${s.location}` : ""}
                          <span className="pill pill-muted" style={{ marginLeft: 6 }}>
                            請假{s.absent_deduct === false ? "(不扣課)" : "(扣課)"}
                          </span>
                        </span>
                        <ChevronRight size={16} className="row-arrow" />
                      </div>
                  ...
                  <div key={d} className="cal-multiday-daycol" style={{ height: `${(CAL_END_HOUR - CAL_START_HOUR) * CAL_HOUR_HEIGHT}px` }}>
                    ...
                    {dList.map((s) => {
                      ...
                      return (
                        <button
                          key={s.id}
                          className={`cal-block cal-block-session ${isTrial ? "trial" : "member"} ${isCompleted ? "completed" : ""} ${isAbsent ? "absent" : ""}`}
                          style={{
                            top: `${top}px`,
                            height: `${height}px`,
                            ...(isAbsent ? { background: "#9ca3af", borderColor: "#6b7280", color: "#fff" } : {}),
...
function CashflowManager({ members, sessions, payments, manualCharges, venues, venuePurchaseRecords, purchaseRecords, miscExpenses, coaches, isStaff, currentCoachId, onAddVenue, onUpdateVenue, onDeleteVenue, onPurchaseVenue, onDeleteVenuePurchaseRecord, onAddMiscExpense, onDeleteMiscExpense }) {
  ...
  const summary = useMemo(() => {
    ...
    const perMember = regularMembers.map((m) => {
      ...
      const totalDue = memberPayments.reduce((sum, p) => sum + Number(p.amount), 0);
      // 手動調整總金額(扣費為正、退費為負),與「繳款」分頁的「剩餘未付」算法保持一致
      const memberManualCharges = (manualCharges || []).filter((c) => c.member_id === m.id);
      const manualAdjustTotal = memberManualCharges.reduce((sum, c) => sum + Number(c.amount || 0), 0);
      const grandTotal = courseFee + manualAdjustTotal;
      // 剩餘未付 = 總金額(課費+手動調整) - 已收金額,計算方式與「繳款」分頁的 remainingUnpaid 一致
      const unpaid = Math.max(grandTotal - allPaid, totalDue + manualAdjustTotal - allPaid, 0);
    ...
    // 未收取課費總額 = 所有學員的剩餘未付總計(逐一加總每位學員的未付金額,
    // 避免某學員溢繳的金額被拿去抵銷其他學員的未付款,更準確反映實際待收餘額)
    const totalUnpaidFromInstallments = perMember.reduce((sum, pm) => sum + pm.unpaid, 0);
...
export default function App() {
  ...
  const saveSession = async (sessionId, payload) => {
    ...
    // 請假扣課:若此堂課會扣課(deduct_session)、且被標記為請假並勾選「請假扣課」,
    // 視同已用掉一堂課,已用堂數需 +1;若原本已因請假扣課而 +1、後來取消請假或改為不扣課,則需退回 -1。
    // 用「先前狀態 vs. 這次儲存後的狀態」比較差異,避免同一堂課被重複扣課或漏退。
    if (target && target.deduct_session) {
      const wasAbsentDeducted = !!(target.is_absent && target.absent_deduct !== false);
      const willAbsentDeduct = !!(update.is_absent && update.absent_deduct !== false);
      if (wasAbsentDeducted !== willAbsentDeduct) {
        const member = members.find((m) => m.id === target.member_id);
        if (member) {
          const delta = willAbsentDeduct ? 1 : -1;
          await supabase
            .from("members")
            .update({ used_sessions: Math.max(member.used_sessions + delta, 0) })
            .eq("id", member.id);
        }
      }
    }
  ...
  const deleteSession = async (sessionId) => {
    ...
    // 已完成、或因請假扣課而已扣掉一堂的課堂,刪除時都需要把堂數退回去
    const shouldRefund =
      target &&
      target.deduct_session &&
      (target.status === "completed" || (target.is_absent && target.absent_deduct !== false));
    if (shouldRefund) {
      const member = members.find((m) => m.id === target.member_id);
  ...
  // 小編是否可以查看金流：教練端的 staff_can_view_cashflow 設定控制
  // 教練本人永遠可以看；小編端看被選教練的設定(預設 true)
  const staffCanViewCashflow = isStaff
    ? (selectedCoach?.staff_can_view_cashflow !== false)
    : true;

  // 過濾掉不可見的 tab(小編被禁止查看金流時隱藏金流 tab)
  const visibleTabs = TAB_CONFIG.filter((t) => {
    if (t.key === "cashflow" && isStaff && !staffCanViewCashflow) return false;
    return true;
  });

  // 教練切換自己的金流小編可見性設定
  const toggleCashflowForStaff = async () => {
    const current = profile?.staff_can_view_cashflow !== false;
    const newVal = !current;
    await supabase.from("profiles").update({ staff_can_view_cashflow: newVal }).eq("id", session.user.id);
    // 同步更新本地 profile state
    setProfile((p) => ({ ...p, staff_can_view_cashflow: newVal }));
  };
  ...
  return (
    <div className="app-shell">
        ...
        <div className="topbar-user">
          ...
          {!isStaff && (
            <button
              ...
              onClick={toggleCashflowForStaff}
              title={profile?.staff_can_view_cashflow !== false ? "點擊可禁止小編查看金流" : "點擊可允許小編查看金流"}
            >
              {profile?.staff_can_view_cashflow !== false ? "🔓 小編可看金流" : "🔒 金流已對小編隱藏"}
            </button>
          )}
      ...
      <main className="main-content">
        ...
        {activeTab === "cashflow" && staffCanViewCashflow && (
          <CashflowManager
            members={scopedMembers}
            sessions={scopedSessions}
            payments={scopedPayments}
