    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal modal-large" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-footer">
          ...
          <button className="btn-primary" onClick={() => handleSave(true)} disabled={saving || !canComplete} title={!canComplete ? "課堂尚未到開始時間,無法標記為完成" : undefined}>
    ...
    <div className="section">
      <div className="stat-grid">
        <div className="stat-card">
          ...
          <span className={`stat-value ${totalCharged >= 0 ? "warn" : "accent"}`}>
      ...
      <div className="hint-box">
        此處用於記錄不透過正常排課/購課流程的金額或堂數調整,例如：臨時取消的場地費、額外服務費、遲到費、課程退費,或單純手動增減課程堂數。金額可選擇「扣費」或「退費」,堂數可選擇「扣除」或「加回」,兩者各自獨立,不需要同時填。
      </div>
      ...
      <div className="stat-grid">
        ...
        {summary.manualChargeTotal !== 0 && (
          <div className="stat-card">
            ...
            <span className={`stat-value ${summary.manualChargeTotal >= 0 ? "warn" : "accent"}`}>
...
export default function App() {
  ...
  const addManualCharge = async (memberId, payload) => {
    ...
    // sessions_deducted 為正值表示扣除堂數,負值表示加回堂數,直接套用到 used_sessions
    // (正值時 used_sessions 增加=消耗堂數;負值時 used_sessions 減少=退回堂數)
    const sessionsDelta = Number(payload.sessions_deducted || 0);
    if (sessionsDelta !== 0 && member) {
      const newUsed = Math.max(member.used_sessions + sessionsDelta, 0);
      await supabase
        .from("members")
  ...
  const deleteManualCharge = async (id) => {
    ...
    // 刪除手動調整紀錄時,需把該筆對 used_sessions 造成的影響還原回去
    const sessionsDelta = Number(target?.sessions_deducted || 0);
    if (sessionsDelta !== 0) {
      const member = members.find((m) => m.id === target.member_id);
      if (member) {
        const restoredUsed = Math.max(member.used_sessions - sessionsDelta, 0);
        await supabase
          .from("members")
          .update({ used_sessions: restoredUsed })
          .eq("id", member.id);
      }
    }
  };
