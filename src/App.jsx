...
function CashflowManager({ members, sessions, payments, manualCharges, venues, venuePurchaseRecords, purchaseRecords, miscExpenses, coaches, isStaff, currentCoachId, onAddVenue, onUpdateVenue, onDeleteVenue, onPurchaseVenue, onDeleteVenuePurchaseRecord, onAddMiscExpense, onDeleteMiscExpense }) {
  ...
  const summary = useMemo(() => {
    ...
    // 已收金額(篩選期間):所有學員已結清的繳款紀錄 + 體驗課課費總額
    // (體驗課課費視為已收,不受年月篩選,因體驗課通常無分期繳款紀錄可篩選)
    const totalPaid = filteredPayments.filter((p) => p.is_paid).reduce((sum, p) => sum + Number(p.amount), 0) + trialFeeTotal;
  ...
  }, [regularMembers, members, payments, filteredPayments, purchaseRecords, filteredManualCharges]);
...
function MiscExpenseSection({ miscExpenses, coaches, isStaff, currentCoachId, onAddMiscExpense, onDeleteMiscExpense }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLabel(""); setAmount(""); setExpenseDate(todayStr()); setNote("");
    setShowForm(false);
  };
  ...
  return (
    <div className="section">
      ...
      <p className="page-sub">記錄不屬於學員課費或場地費的其他支出,例如停車費、油錢、材料費等</p>
      ...
      {showForm && (
        <form className="card form-card" onSubmit={handleSubmit}>
          <label>
            品項
            <input type="text" value={label} onChange={(e) => setLabel(e.target.value)} placeholder="例如:停車費、油錢、材料費" required />
          </label>
          <div className="form-grid">
            <label>
              金額(元)
