...
function Overview({ members, sessions, payments, blockedTimes, onGoSchedule, onOpenSession, onOpenMember }) {
  ...
  const stats = useMemo(() => {
    ...
    // 本週上課堂數:計算一週已安排課程(排定中 + 已完成),含請假課程,僅排除已取消的課堂
  ...
  // 付款提醒 map:member_id -> 最迫切的一筆(用於直接標示在當日行程色塊上)
  const paymentReminderMap = useMemo(() => {
    const map = {};
    paymentReminders.forEach(({ payment, due }) => {
      if (!map[payment.member_id]) {
        map[payment.member_id] = { amount: payment.amount, due };
      }
    });
    return map;
  }, [paymentReminders]);
        ...
        <div className="overview-timeline-wrap">
          <div className="overview-timeline" style={{ height: `${15 * CAL_H}px` }}>
            ...
            {todayBlocks.map((b, i) => {
              ...
              const bottom = timeToTop(b.end_time);
              const height = Math.max(bottom - top, 20);
              return (
                <div
                  key={`block-${b.id || i}`}
                  className="ovt-block-unavailable"
            ...
            {todaySessions.map((s) => {
              ...
              const paymentAlert = paymentReminderMap[s.member_id];
              ...
              return (
                ...
                >
                  <div className="ovt-block-toprow">
                    ...
                    {paymentAlert && (
                      <span className="ovt-block-payment-alert">
                        待收款 {fmtMoney(paymentAlert.amount)}
                      </span>
                    )}
                  </div>
...
function CalendarView({ members, sessions, blockedTimes, isStaff, currentCoachId, coaches, onOpenSession, onAddBlockedTime, onUpdateBlockedTime, onDeleteBlockedTime, searchQuery, onSearchChange }) {
  ...
  // 下方「已設定的不可排課時段」清單只顯示尚未過去的:
  // 單次日期的時段若日期已過(或當天但結束時間已過)則不顯示;每週固定的時段為長期規則,永遠顯示
  const upcomingBlockedTimes = useMemo(() => {
    const nowDateStr = todayStr();
    const now = new Date();
    const nowMinutes = now.getHours() * 60 + now.getMinutes();
    return visibleBlockedTimes.filter((b) => {
      if (!b.block_date) return true; // 每週固定規則,永遠顯示
      if (b.block_date < nowDateStr) return false;
      if (b.block_date === nowDateStr) {
                  ...
                  <div key={d} className="cal-multiday-daycol" style={{ height: `${(CAL_END_HOUR - CAL_START_HOUR) * CAL_HOUR_HEIGHT}px` }}>
                    ...
                    {dBlocks.map((b, i) => {
                      ...
                      return (
                        <button
                          key={`block-${b.id || i}`}
                          type="button"
                          className="cal-block cal-block-unavailable cal-block-unavailable-clickable"
                          style={{ top: `${top}px`, height: `${height}px` }}
                    ...
                    {dList.map((s) => {
                      ...
                      return (
                        <button
                          key={s.id}
                          className={`cal-block cal-block-session ${isTrial ? "trial" : "member"} ${isCompleted ? "completed" : ""} ${s.is_absent ? "absent" : ""}`}
                          style={{ top: `${top}px`, height: `${height}px` }}
                          onClick={() => onOpenSession(s)}
                        >
      ...
      {editingBlock && (
        <BlockedTimeEditModal
          ...
          onSave={async (updates) => {
            await onUpdateBlockedTime(editingBlock.id, updates);
            setEditingBlock(null);
          }}
          onDelete={async () => {
            await onDeleteBlockedTime(editingBlock.id);
            setEditingBlock(null);
          }}
        />
      )}
...
/* ---------- 行事曆上點擊不可排課時段色塊後的編輯 Modal ---------- */
function BlockedTimeEditModal({ block: b, onClose, onSave, onDelete }) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    await onSave({ label: label.trim(), start_time: startTime, end_time: endTime });
    setSaving(false);
  };
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        <div className="modal-head">
          <div>
            <h3>調整不可排課時段</h3>
            <p className="modal-sub">
              {b.weekday !== null && b.weekday !== undefined ? `每週${WEEKDAY_LABELS[b.weekday]}` : fmtDate(b.block_date)}
            </p>
          </div>
          <button className="icon-btn" onClick={onClose}>
            <X size={18} />
        ...
        <div className="modal-footer">
          {!confirmDelete ? (
            ...
            <div className="confirm-delete">
              <span>確定刪除此不可排課時段?</span>
              <button className="btn-secondary" onClick={() => setConfirmDelete(false)}>取消</button>
              <button className="btn-danger" onClick={onDelete}>確認刪除</button>
            </div>
          )}
          <div className="spacer" />
