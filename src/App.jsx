...
/* ============================================================
   排課管理
   ============================================================ */
function ScheduleManager({ members, sessions, onAdd, onOpenSession, presetMemberId, onPresetUsed, venues, blockedTimes, searchQuery, onSearchChange }) {
...
/* ---------- 基本資料 Tab ---------- */
function ProfileTab({ member, periodRecords, memberPhotos, onUpdateMember, onAddPeriodRecord, onDeletePeriodRecord, onAddMemberPhoto, onDeleteMemberPhoto, onUpdateMemberPhoto }) {
  const [form, setForm] = useState({
...
function ManualChargesTab({ member, manualCharges, onAddManualCharge, onDeleteManualCharge }) {
  ...
  const [amount, setAmount] = useState("");
  const [sessionsDeducted, setSessionsDeducted] = useState("");
  const [note, setNote] = useState("");
  ...
  const handleSubmit = async (e) => {
    ...
    const sessionsNum = Number(sessionsDeducted) || 0;
    if (amountNum <= 0 && sessionsNum <= 0) {
      setError("請至少填寫金額或扣除堂數其中一項");
      return;
    }
    if (sessionsNum > 0 && member.member_type === "member") {
      const remaining = Math.max(member.total_sessions - member.used_sessions, 0);
      if (sessionsNum > remaining) {
        setError(`此學員剩餘堂數不足(剩餘 ${remaining} 堂),無法扣除 ${sessionsNum} 堂`);
        return;
      }
    }
    await onAddManualCharge(member.id, {
      label: label.trim(),
      amount: amountNum,
      sessions_deducted: sessionsNum,
      charge_date: chargeDate,
      note: note.trim(),
    });
    setLabel(""); setAmount(""); setSessionsDeducted(""); setNote(""); setChargeDate(todayStr());
    setShowForm(false);
  };
    ...
    <div className="section">
      <div className="stat-grid">
        ...
        {totalSessionsDeducted > 0 && (
          <div className="stat-card">
            <span className="stat-label">累計手動扣除堂數</span>
            <span className="stat-value warn">{totalSessionsDeducted} 堂</span>
          </div>
        )}
      </div>

      <div className="hint-box">
        此處用於記錄不透過正常排課流程的扣費或扣堂數情況,例如：臨時取消的場地費、額外服務費、遲到費,或單純手動扣除課程堂數。金額會同步顯示在「金流」分頁;扣除堂數會直接從學員剩餘堂數中扣除,兩者可分別獨立填寫,不需要同時填。
      </div>
      ...
      {showForm && (
        <form className="card form-card" onSubmit={handleSubmit}>
          ...
          <div className="trial-fields-row">
            ...
            <label className="block-label">
              扣除堂數(選填)
              <input type="number" min={0} value={sessionsDeducted} onChange={(e) => setSessionsDeducted(e.target.value)} placeholder="0" />
            </label>
          </div>
          <label className="block-label">
            日期
            ...
            <div className="payment-row" key={c.id}>
              <div className="list-row-main">
                ...
                <span className="list-row-sub">
                  ...
                  {c.sessions_deducted > 0 ? ` · 扣除 ${c.sessions_deducted} 堂` : ""}
                  {c.note ? ` · ${c.note}` : ""}
                </span>
              </div>
...
function CalendarView({ members, sessions, blockedTimes, isStaff, currentCoachId, coaches, onOpenSession, onAddBlockedTime, onDeleteBlockedTime, searchQuery, onSearchChange }) {
  const [anchorDate, setAnchorDate] = useState(todayStr());
  // 檢視天數:1 = 單日, 3 = 三日, 7 = 七日(預設)
  ...
  const rangeLabel = useMemo(() => {
    ...
    if (viewDays === 1) {
      return `${first.slice(0, 4)} 年 ${Number(first.slice(5, 7))} 月 ${Number(first.slice(8, 10))} 日(${WEEKDAY_LABELS[weekdayOf(first)]})`;
    }
    return `${first.slice(0, 4)} 年 ${Number(first.slice(5, 7))} 月 ${Number(first.slice(8, 10))} 日 - ${Number(last.slice(8, 10))} 日`;
  }, [rangeDates, viewDays]);

  const stepBack = () => setAnchorDate((d) => dateStrAdd(d, -viewDays));
  const stepForward = () => setAnchorDate((d) => dateStrAdd(d, viewDays));
  const goToday = () => setAnchorDate(todayStr());
  ...
  return (
    <div className="page">
      ...
      <div className="cal-view-toggle">
        {[
          ...
          <button
            key={opt.v}
            type="button"
            className={`cal-view-toggle-btn ${viewDays === opt.v ? "active" : ""}`}
            onClick={() => setViewDays(opt.v)}
          >
            {opt.label}
          </button>
        ))}
      ...
      <div className="section">
        ...
        <div className={`cal-multiday-wrap ${viewDays === 1 ? "single" : ""}`}>
          <div className="cal-multiday-scrollx">
            {/* 欄頭:各天的日期標籤 */}
            <div className="cal-multiday-header" style={{ gridTemplateColumns: `48px repeat(${viewDays}, 1fr)` }}>
              <div className="cal-multiday-corner" />
              {rangeDates.map((d) => {
                const wd = weekdayOf(d);
                const isToday = d === today;
                return (
                  <div key={d} className={`cal-multiday-daylabel ${isToday ? "today" : ""}`}>
            ...
            {/* 時間軸主體:左側時間刻度 + 每天一欄 */}
            <div className="cal-multiday-body" style={{ gridTemplateColumns: `48px repeat(${viewDays}, 1fr)` }}>
...
/* ============================================================
   金流紀錄
   ============================================================ */
function CashflowManager({ members, sessions, payments, manualCharges, venues, coaches, isStaff, currentCoachId, onAddVenue, onUpdateVenue, onDeleteVenue, onPurchaseVenue }) {
  ...
  const summary = useMemo(() => {
    ...
    const perMember = regularMembers.map((m) => {
      ...
      const memberFilteredPayments = filteredPayments.filter((p) => p.member_id === m.id);
      const paid = memberFilteredPayments.filter((p) => p.is_paid).reduce((sum, p) => sum + Number(p.amount), 0);
      ...
      const unpaid = Math.max(courseFee - allPaid, installmentTotal - allPaid, 0);
  ...
  }, [regularMembers, trialMembers, members, payments, filteredPayments, manualCharges]);
  ...
  return (
    <div className="page">
      ...
      {(filterYear || filterMonth) && (
        <div className="hint-box">
          「已收金額」相關統計目前僅顯示應付日落在 {filterYear ? `${filterYear}年` : ""}{filterMonth ? `${Number(filterMonth)}月` : ""} 且已結清的紀錄,其餘課費總額、未收金額等統計不受篩選影響。
        </div>
      )}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const loadAll = async () => {
      ...
      if (cancelled) return;
      if (mErr || sErr || pErr || bErr || vErr || btErr || mcErr || prErr || mpErr) {
        setLoadError((mErr || sErr || pErr || bErr || vErr || btErr || mcErr || prErr || mpErr).message);
        return;
      }
  ...
  const addManualCharge = async (memberId, payload) => {
    ...
    // 若有設定扣除堂數,直接從學員 used_sessions 累加(等同消耗該堂數)
    if (payload.sessions_deducted && payload.sessions_deducted > 0 && member) {
      await supabase
        .from("members")
        .update({ used_sessions: member.used_sessions + Number(payload.sessions_deducted) })
        .eq("id", memberId);
    }
  };
  ...
  const deleteManualCharge = async (id) => {
    ...
    // 刪除手動扣費紀錄時,若該筆曾扣除堂數,需退回給學員
    if (target && target.sessions_deducted > 0) {
      const member = members.find((m) => m.id === target.member_id);
      if (member && member.used_sessions > 0) {
        await supabase
          .from("members")
          .update({ used_sessions: Math.max(member.used_sessions - target.sessions_deducted, 0) })
          .eq("id", member.id);
      }
    }
  };
