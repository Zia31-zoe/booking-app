      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="form-card">
          ...
          <p className="modal-hint">
            ...
            {calcAmount > 0 && ` · 收款金額 $${calcAmount.toLocaleString()}`}
          </p>
          <button type="submit" className="btn-primary full">
            確認購課
          </button>
...
function PaymentTab({ member, payments, totalPaid, totalCourseFee, manualAdjustTotal, grandTotal, remainingUnpaid, onAddPayment, onUpdatePayment, onDeletePayment }) {
  const [showForm, setShowForm] = useState(false);
...
/* ============================================================
   金流紀錄
   ============================================================ */
function CashflowManager({ members, sessions, payments, manualCharges, venues, venuePurchaseRecords, coaches, isStaff, currentCoachId, onAddVenue, onUpdateVenue, onDeleteVenue, onPurchaseVenue, onDeleteVenuePurchaseRecord }) {
  ...
  }, [regularMembers, trialMembers, members, payments, filteredPayments, manualCharges, filteredManualCharges]);
  ...
  return (
    <div className="page">
      ...
      {(filterYear || filterMonth) && (
        <div className="hint-box">
          「已收金額」「手動調整扣費」「手動調整退費」「場地總支出金額」目前僅顯示落在 {filterYear ? `${filterYear}年` : ""}{filterMonth ? `${Number(filterMonth)}月` : ""} 範圍內的紀錄,其餘課費總額、未收金額等統計不受篩選影響。
        </div>
      )}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const loadAll = async () => {
      ...
      if (cancelled) return;
      if (mErr || sErr || pErr || bErr || vErr || btErr || mcErr || prErr || mpErr || purErr || vprErr) {
        setLoadError((mErr || sErr || pErr || bErr || vErr || btErr || mcErr || prErr || mpErr || purErr || vprErr).message);
        return;
      }
  ...
  const purchaseMore = async (memberId, sessionsAdded, amount = 0, newPricePerSession = null, purchaseDate = null) => {
    ...
    await supabase
      ...
      .eq("id", memberId);
    // 寫入購課歷史紀錄(購課日期、堂數、單堂金額),供「基本資料」分頁的購課紀錄列表顯示
    await supabase.from("purchase_records").insert({
      member_id: memberId,
      coach_id: member.coach_id,
      purchase_date: dateStr,
      sessions_added: sessionsAdded,
      price_per_session: newPricePerSession !== null ? newPricePerSession : member.price_per_session || 0,
      amount,
    });
    if (amount && amount > 0) {
