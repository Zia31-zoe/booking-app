      ...
      >
        <div className="list-row-main">
          ...
          <span className="list-row-sub">
            {(s.start_time || "").slice(0, 5)} · {s.duration_min} 分鐘
            {s.location ? ` · ${s.location}` : ""}
          </span>
          {s.coach_notes && <span className="list-row-sub list-row-coach-notes">教練筆記:{s.coach_notes}</span>}
        </div>
        <ChevronRight size={18} className="row-arrow" />
      </button>
    );
    ...
    <div className="page">
      ...
      <div className="stat-grid">
        ...
        <div className="stat-card">
          ...
          <span className="stat-value">{stats.activeMemberCount}</span>
        </div>
      ...
      {showTrialDetail && (
        <MonthlyListModal
          title="本月新增體驗課"
          onClose={() => setShowTrialDetail(false)}
          items={stats.monthlyTrials}
          onOpenMember={onOpenMember}
          renderItem={(m) => {
            const trialSession = sessions.find((s) => s.member_id === m.id);
            const loc = trialSession?.location || "";
            return (
              <span className="list-row-sub">
                {m.trial_session_date ? `${fmtDate(m.trial_session_date)} ${m.trial_start_time || ""}` : "尚未排課"}
                {loc ? ` · ${loc}` : ""}
              </span>
            );
          }}
      ...
      {showConvertedDetail && (
        <MonthlyListModal
          title="本月新增學員"
          onClose={() => setShowConvertedDetail(false)}
          items={stats.monthlyConverted}
          onOpenMember={onOpenMember}
          renderItem={(m) => (
            <span className="list-row-sub">
              轉為正式學員時間:{m.converted_at ? `${fmtDate(m.converted_at.slice(0, 10))}` : "未知"}
            </span>
          )}
        />
      )}
