...
const TAB_CONFIG = [
  ...
  { key: "logs", label: "訓練紀錄", icon: ClipboardList },
...
function Overview({ members, sessions, payments, onGoSchedule, onOpenSession, onOpenMember }) {
  ...
  const memberAlerts = useMemo(() => {
    ...
    members.forEach((m) => {
      ...
      if (inbody) alerts.push({ member: m, type: "inbody", status: inbody });
      if (period) alerts.push({ member: m, type: "period", status: period });
    });
    return alerts;
              ...
              >
                <div className="list-row-main">
                  <span className="list-row-title alert-text">
                    {a.member.name}
                    {a.type === "inbody" ? "  需拍攝 Inbody / 體態照" : "  生理期將至,請留意調整訓練"}
                  </span>
                  <span className="list-row-sub">
                    {a.type === "inbody"
                      ? a.status === "due"
...
/* ============================================================
   排課管理
   ============================================================ */
function ScheduleManager({ members, sessions, onAdd, onOpenSession, presetMemberId, onPresetUsed, commonLocations }) {
          ...
          {trialMembers.map((m) => (
            <div className="member-card trial" key={m.id}>
              <div className="member-card-head">
                ...
                <span className={`pill ${m.trial_followup_status === "converted" ? "pill-free" : m.trial_followup_status === "declined" ? "pill-warn" : "pill-trial"}`}>
                  {m.trial_followup_status === "converted" ? "已轉正式" : m.trial_followup_status === "declined" ? "未簽約" : "追蹤中"}
                </span>
              </div>
              <div className="member-phone">
      ...
      <div className="modal modal-large" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div className="followup-options">
            <button
              type="button"
              className={`followup-btn ${form.trial_followup_status === "converted" ? "active success" : ""}`}
            ...
            <button
              type="button"
              className={`followup-btn ${form.trial_followup_status === "declined" ? "active danger" : ""}`}
              onClick={() => update("trial_followup_status", "declined")}
            >
              <XCircle size={16} /> 不願簽約
            </button>
            <button
              type="button"
              className={`followup-btn ${form.trial_followup_status === "pending" ? "active" : ""}`}
              onClick={() => update("trial_followup_status", "pending")}
            >
              <Clock size={16} /> 持續追蹤
            </button>
          </div>

          {form.trial_followup_status === "declined" && (
            <label className="block-label">
              不願簽約原因
              <textarea rows={2} value={form.trial_followup_reason} onChange={(e) => update("trial_followup_reason", e.target.value)} />
            </label>
          )}
...
/* ============================================================
   訓練紀錄(依學員區分)
   ============================================================ */
function LogsList({ members, sessions, onOpenSession, onOpenMember }) {
  ...
  const grouped = useMemo(() => {
    ...
    Object.values(map).forEach((list) =>
      list.sort((a, b) => (b.session_date + b.start_time).localeCompare(a.session_date + a.start_time))
    );
    return map;
  }, [sessions]);

  const memberList = members
    .filter((m) => grouped[m.id])
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div>
          <h1>訓練紀錄</h1>
          <p className="page-sub">依學員瀏覽已完成課堂的訓練內容與學員回饋</p>
        </div>
      </div>

      {memberList.length === 0 ? (
        <div className="empty-block">目前還沒有完成的課堂紀錄。</div>
      ) : (
        <div className="list">
          {memberList.map((m) => {
            ...
            return (
              <div key={m.id}>
                <button className="list-row clickable" onClick={() => setExpandedMemberId(expanded ? null : m.id)}>
                  <div className="list-row-main">
                    <span className="list-row-title">
                      {m.name}
                      {m.member_type === "trial" && <span className="pill pill-trial">體驗課</span>}
                    </span>
                    <span className="list-row-sub">{list.length} 筆已完成課堂</span>
                  </div>
                  <ChevronRight size={18} className={`row-arrow ${expanded ? "rotated" : ""}`} />
                </button>
                {expanded && (
                  <div className="logs-sublist">
                    {list.map((s) => (
                      <div className="history-row" key={s.id}>
                        <div className="history-row-head">
                          <span className="list-row-title">
                          ...
                          {s.energy_rating ? (
                            ...
                            <span className="pill pill-muted">尚未填寫</span>
                          )}
                        </div>
                        {s.warmup && <p className="history-content"><strong>熱身:</strong>{s.warmup}</p>}
                        {s.main_training && <p className="history-content"><strong>主訓練:</strong>{s.main_training}</p>}
                        ...
                        {s.coach_notes && <p className="history-content"><strong>教練筆記:</strong>{s.coach_notes}</p>}
                        {s.member_feedback && <p className="history-content"><strong>學員回饋:</strong>{s.member_feedback}</p>}
                        <button className="btn-secondary small" onClick={() => onOpenSession(s)}>
                          開啟編輯
                        </button>
                      </div>
                    ))}
...
/* ============================================================
   金流紀錄
   ============================================================ */
function CashflowManager({ members, payments }) {
  const [expandedMemberId, setExpandedMemberId] = useState(null);
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const loadAll = async () => {
      ...
      if (cancelled) return;
      if (mErr || sErr || pErr || bErr) {
        setLoadError((mErr || sErr || pErr || bErr).message);
        return;
      }
      setMembers(m || []);
      setSessions(s || []);
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        {activeTab === "logs" && (
          <LogsList members={members} sessions={sessions} onOpenSession={setActiveSession} />
        )}
        {activeTab === "cashflow" && <CashflowManager members={members} payments={payments} />}
      </main>
